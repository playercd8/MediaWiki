...
class SkinMonaco extends SkinTemplate {
	...
	public function tooltipAndAccesskeyAttribs( $name ) {
		...
		$attribs = array(
			'title' => Linker::titleAttrib( $name, 'withaccess' ),
			'accesskey' => Linker::accesskey( $name )
		);
...
?>
					...
					echo wfMessage('monaco-footer-lastedit')->rawParams(Linker::link($userPageTitle, htmlspecialchars($user->getName()), array( "id" => "fe_user_link" )), Html::element('time', array( 'datetime' => wfTimestamp( TS_ISO_8601, $$timestamp )), $lastUpdate))->escaped() ?></div></li>
<?php
			}
		}
		...
		<div id="widget_sidebar" class="reset widget_sidebar left_sidebar sidebar">
			...
			<style type="text/css">
				#wiki_logo {
					...
					filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo escapeJsString( $this->data['logopath'] ) ?>', sizingMethod='image');
				}
			</style>
			<![endif]-->
...
<?php
				...
				<span>
					<?php 
					echo Html::element( 'a', array( 'href' => $this->data['userlinks']['logout']['href'] ) + $skin->tooltipAndAccesskeyAttribs('pt-logout'), $this->data['userlinks']['logout']['text'] ); 
					?>
